...
# --------------------------------------------------------------------------
# Constants:
TOP_250_URL = 'http://www.top40.nl/top40/1965/week-51' # voor een array
...
def scrape_movie_page(dom):
    ...
    for t in dom.by_tag('span.itemprop')[:1]:
        title = t.content
    #for y in dom.by_tag('span.nobr')[:1]:
        #year = y.content
        #print year
    for e in dom.by_tag('div.infobar')[:1]:
        #print e
        for l in e.by_tag('time'):
            l = l.content
            k = l.split('\t')
            duration = k[0].strip()
        for genre in e.by_tag('span.itemprop'):
            stripgenre = genre.content
            listgenres.append(stripgenre)
            genres = ";".join(listgenres)
    # get the director
    for b in dom.by_tag('div.txt-block')[0:1]:
        for director in b.by_tag('span.itemprop'):
            director = director.content

            listdirectors.append(director)
            directors = ";".join(listdirectors)

    for v in dom.by_tag('div.txt-block')[1:2]:
        for writer in v.by_tag('span.itemprop'):
            writer = writer.content
            listwriters.append(writer)
            writers = ";".join(listwriters)
    # get the actors(stars)
    for a in dom.by_tag('div.txt-block')[2:3]:

        for director in a.by_tag('span.itemprop'):
            i = director.content
            listactors.append(i)
            actors = ";".join(listactors)
    # get the rating
    for p in dom.by_tag('div.star-box-details'):

        for ratings in p.by_tag('span')[0:1]:
